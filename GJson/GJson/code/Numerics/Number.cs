...
namespace Gal.Core
{
    ...
    public struct Number
    {
        ...
        public static Number operator -(Number v) => v.m_IsDouble ? -(double)v : -(long)v;

        public static Number operator +(Number a, Number b) =>
            a.m_IsDouble ? b.m_IsDouble ? (double)a + (double)b : (double)a + (long)b : b.m_IsDouble ? (long)a + (double)b : (long)a + (long)b;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int ConvertToInt32(ReadOnlySpan<char> span, int fromBase) {
            var result = 0;
            foreach (var c in span) result = result * fromBase + (c - '0');
            return result;
        }
    }
}