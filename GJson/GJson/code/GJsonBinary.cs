...
namespace Gal.Core.GJson
{
    ...
    public static class GJsonBinary
    {
        ...
        public static GJsonObject Decode(ref RefReader<byte> reader) {
            ...
            switch (type) {
                case GJsonType.Long: return new(ReadVarLong(ref reader, header >> 3));
                case GJsonType.String: return new(reader.ReadUtf8(ReadVarLength(ref reader, header)));
                case GJsonType.Null: return new(GJsonType.Null);
                case GJsonType.Object: {
                    GJsonObject json = new(GJsonType.Object);
                ...
                case GJsonType.Array: {
                    GJsonObject json = new(GJsonType.Array);
                    var count = ReadVarLength(ref reader, header);
                    for (var i = 0; i < count; i++) json.Add(Decode(ref reader));
                    return json;
                }
                case GJsonType.Double: return new(reader.ReadDouble());
                case GJsonType.Boolean: return new(header >> 3 != 0);
                default: throw new ArgumentOutOfRangeException();
            }
        }
